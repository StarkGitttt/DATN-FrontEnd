...
function HistoryOrder() {
   ...
   const calcTotalProductSold = (orderHistory) => {
      return orderHistory.orderItems.reduce((accumulator, currentValue) => {
   ...
   useEffect(() => {
      if (userInfo?.id) {
         OrderAPI.findByStatus({
            params: {
               status: constants_local.STATUS_SUCCESS_ORDER,
               curPage: page,
            },
         })
            .then((res) => {
               if (res.data) {
                  console.log('History order', res.data);
            ...
            .catch((e) => {
               console.log('Get order by status history failed: ', e);
            });
      }
   }, [userInfo?.id, page]);
               ...
               <>
                  <div className={cx('limiter')}>
                     <div className={cx('title-box')}>
                        <h3 className="title">{t('order.history.title')}</h3>
                     </div>
