   ...
   useEffect(() => {
      if (userInfo?.id) {
         OrderAPI.findByStatus({
            params: {
               status: constants_local.STATUS_CANCEL_ORDER,
               curPage: page,
            },
         })
            .then((res) => {
               if (res.data) {
                  ...
                  setOrdersCancel(res.data?.content);
                  setPageable(res.data?.pageable);
                  setTotalItems(res.data?.totalElements);
               }
            })
            .catch((e) => {
               console.log('Get order by status cancel failed: ', e);
            });
      }
   }, [userInfo?.id, page]);
               ...
               <>
                  <div className={cx('limiter')}>
                     <div className={cx('title-box')}>
                        <h3 className="title">{t('order.cancel.title')}</h3>
                     </div>
